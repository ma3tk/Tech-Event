...
async function globalSetup(): Promise<void> {
  ...
  // Nx 化で e2e は apps/web-e2e/ で実行されるが、dev.db / prisma schema は
  // apps/web/ に置かれている。`__dirname` から `apps/web/` 絶対パスを引く。
  const e2eRoot = path.resolve(__dirname, ".."); // apps/web-e2e
  const webRoot = path.resolve(e2eRoot, "../web"); // apps/web
  ...
  const seedTestUserPath = path.join(webRoot, "prisma", "seed-test-user.ts");
