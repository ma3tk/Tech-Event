 ...
 *
 * - `{nickname}` をフォローしているユーザー (active のみ) を新しい順に一覧表示。
...
export default async function UserFollowersPage({ params }: PageProps) {
  ...
  const rows = await prisma.follow.findMany({
    where: { followeeId: user.id, follower: { status: "active" } },
      ...
      <div className="mx-auto w-full max-w-3xl px-6 py-8">
        ...
        <h1 className="mt-3 text-2xl font-bold">
          ...
          <span className="text-sm font-normal text-muted-foreground">
            ({formatNumber(user.followerCount)} 人)
          </span>
        </h1>
        ...
        {followers.length === 0 ? (
          ...
          <ul className="mt-6 space-y-3" data-testid="followers-list">
            {followers.map((u) => (
              <li key={u.id}>
                <Link
