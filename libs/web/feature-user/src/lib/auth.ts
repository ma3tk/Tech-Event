...
import {
  ...
  getNextAuthSessionFetcher,
} from "@tech-event/shared-util-auth-session";
...
export async function getCurrentUser(): Promise<User | null> {
  // 1) next-auth のセッション
  // `@/auth` (apps/web/auth.ts) は app レイヤーのため、boundary 違反を避ける
  // 目的で動的 import (`loadNextAuth()`) を経由する。
  let userIdFromAuthJs: bigint | null = null;
  try {
    const fetcher = getNextAuthSessionFetcher();
    if (fetcher) {
